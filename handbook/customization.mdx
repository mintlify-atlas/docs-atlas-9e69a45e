...
- `reason` is used to determine why the change event occurred, which can be useful to conditionally run certain side effects. Most IDEs show the possible string values after typing `reason === '`.
...
In most components, pressing the <kbd>Esc</kbd> key stops the propagation of the event so parent popups don't close simultaneously. This can also be customized with the `allowPropagation()` method:
...
For instance, you can open [Dialog](/components/dialog) after a timeout, without a trigger:
